...
parser.add_option("-i", "--interactive", action='store_true', help="Execute commands from FILE, then become interactive")
...
class Shell(cmd.Cmd):
    ...
    def __init__(self, hostname, port, color=False,
                 username=None, password=None, encoding=None, stdin=None, tty=True, interactive=False,
    ...
    def cmdloop(self):
        ...
        with self.prepare_loop():
            while not self.stop:
                try:
                    if not self.single_statement is None:
...
def read_options(cmdlineargs, environment):
    ...
    if not options.interactive and (options.file or options.execute):
        options.tty = False
